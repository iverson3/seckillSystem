...
func worker(n int, m int, reNameList *sync.Map, res [16]int, sum int, needDone bool) {
	for i := m; i < arrLen - 15 + n; i++ {
		...
		if !in_arr(reNameList, names[i]){
			reNameList.Store(names[i], struct{}{})
			...
			if n < 15 {
				...
				if n == 0 {
					wg.Add(1)
					go worker(n + 1, i + 1, &tmpMap, res, sum + scores[i], true)
				} else {
					wg.Add(1)
					go worker(n + 1, i + 1, &tmpMap, res, sum + scores[i], true)
				}
...
func in_arr(arr *sync.Map, str string) bool {
	if _, ok := arr.Load(str); ok {
		return true
	}
	return false
}
